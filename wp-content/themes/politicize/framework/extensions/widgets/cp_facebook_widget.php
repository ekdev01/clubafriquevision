...
class facebook_widget extends WP_Widget
{
  function facebook_widget()
  {
    ...
    parent::__construct('facebook_widget', 'CrunchPress : Facebook', $widget_ops);
  }
...
?>
  ...
  <label for="<?php echo $this->get_field_id('pageurl'); ?>">
	  <?php _e('Page URL:','crunchpress');?> 
	  <input class="widefat" id="<?php echo $this->get_field_id('pageurl'); ?>" size='40' name="<?php echo $this->get_field_name('pageurl'); ?>" type="text" value="<?php echo esc_attr($pageurl); ?>" />
	<br />
      <small><?php _e('Please enter your page url example: http://www.facebook.com/profilename OR','crunchpress');?> <br />
     <?php _e('https://www.facebook.com/pages/wxyz/123456789101112','crunchpress');?> 
	</small><br />
  </label>
  </p> 
  <p>
  <label for="<?php echo $this->get_field_id('showfaces'); ?>">
	 <?php _e('Show Faces:','crunchpress');?> 
	  <select id="<?php echo $this->get_field_id('showfaces'); ?>" name="<?php echo $this->get_field_name('showfaces'); ?>" class="widefat">
			<option <?php if($showfaces == 'true'){echo 'selected';}?> value="true"><?php _e('Yes','crunchpress');?></option>
			<option <?php if($showfaces == 'false'){echo 'selected';}?> value="false"><?php _e('No','crunchpress');?></option>
      </select>
  </label>
  </p>  
  <p>
  <label for="<?php echo $this->get_field_id('showstream'); ?>">
	  <?php _e('Show Stream:','crunchpress');?> 
	   <select id="<?php echo $this->get_field_id('showstream'); ?>" name="<?php echo $this->get_field_name('showstream'); ?>" class="widefat">
			<option <?php if($showstream == 'true'){echo 'selected';}?> value="true"><?php _e('Yes','crunchpress');?></option>
			<option <?php if($showstream == 'false'){echo 'selected';}?> value="false"><?php _e('No','crunchpress');?></option>
      </select>
  </label>
  </p> 
  <p>
  <label for="<?php echo $this->get_field_id('likebox_width'); ?>">
	  <?php _e('Like Box Width:','crunchpress');?>
	  <input class="widefat" id="<?php echo $this->get_field_id('likebox_width'); ?>" size='2' name="<?php echo $this->get_field_name('likebox_width'); ?>" type="text" value="<?php echo esc_attr($likebox_width); ?>" />
  </label>
  </p>
  <p>
  <label for="<?php echo $this->get_field_id('likebox_height'); ?>">
	  <?php _e('Like Box Height:','crunchpress');?>
	  <input class="widefat" id="<?php echo $this->get_field_id('likebox_height'); ?>" size='2' name="<?php echo $this->get_field_name('likebox_height'); ?>" type="text" value="<?php echo esc_attr($likebox_height); ?>" />
  </label>
  </p>
<?php
  }
